...
public class OozieClientITV40 extends OozieClientIT{
     ...
     */
    @Test
    public void testShellELParsingErrorHandleV40() {
        try {
            Properties configs = getDefaultProperties();

            // ehco
            String appName = "shell-el-error";
            String version = "v40";
            String appPath = baseAppPath + "/" + version + "/" + appName;
            configs.put(OozieClient.APP_PATH, appPath);
            configs.put("appName", appName);
            ...
            try {
                for (int i = 0; i < 50; i++) {
                    ...
                    for (WorkflowAction action : actionList) {
                        if(action.getName().equals("shell-v40")){
                            LOG.debug("    " + action.getName() + " [" + action.getStatus().toString() + "]");
                            actionStatus = action.getStatus().toString();
                        }
                    }
                    status = wfJob.getStatus().toString();
                    if (wfJob.getStatus().equals(WorkflowJob.Status.SUCCEEDED)
                    ...
                    if (actionStatus.equals(WorkflowAction.Status.START_MANUAL.toString())) {
                        break;
                    }
                    Thread.sleep(POLLING);
                }
            ...
            Assert.assertEquals(WorkflowJob.Status.RUNNING.toString(), status);
            Assert.assertEquals(WorkflowAction.Status.START_MANUAL.toString(), actionStatus);

        } catch (Exception e) {
            LOG.info("Fail to testShellELErrorHandleV40", e);
            Assert.fail();
        }
        LOG.info("    >>>> Pass testShellELErrorHandleV40 \n");
    }
