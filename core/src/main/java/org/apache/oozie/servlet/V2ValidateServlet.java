...
public class V2ValidateServlet extends JsonRestServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (file.startsWith("hdfs://")) {
            try {
                ...
                writeXML(fs.open(path), stringWriter);

            } catch (Exception e) {
                throw new XServletException(HttpServletResponse.SC_BAD_REQUEST, ErrorCode.E0110,
                        "File does not exist, "+ file);
            }
        }
        else {
            writeXML(request.getInputStream(), stringWriter);
        }
        try {
    ...
    private void writeXML(InputStream is, StringWriter writer) throws IOException{
