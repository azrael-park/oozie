...
public class StoreStatusFilter {
    ...
    public static void filter(Map<String, List<String>> filter, List<String> orArray, List<String> colArray,
                              List<String> valArray, StringBuilder sb, String seletStr, FILTER jobFilter) {
        int index = 0;
        ...
        for (Map.Entry<String, List<String>> entry : filter.entrySet()) {
            ...
            String colName = jobFilter.getColName(entry.getKey());
    ...
    public static Map<String, List<String>> parseFilter(String filter, FILTER jobFilter) throws BaseEngineException{
        ...
        for (String condition: filter.split(";")) {
            ...
            if (!jobFilter.isValidFilterName(pair[0].toLowerCase())) {
                throw new BaseEngineException(ErrorCode.E0420, filter, XLog.format("invalid name [{0}]", pair[0]));
            }
            if (pair[0].equals("status")) {
                try {
                    ...
                    throw new BaseEngineException(ErrorCode.E0420, filter, XLog.format("invalid status [{0}]", pair[1]));
                }
            }
    ...
    public static enum FILTER {
        WF{
            public Set<String> getFilterNames() {
                ...
                filterNames.add(OozieClient.FILTER_PARENTID);
                return filterNames;
            }
            public void checkStatus(String status) {
                WorkflowJob.Status.valueOf(status);
            }
            public String getColName(String filterName) {
                if (filterName.equals(OozieClient.FILTER_PARENTID)) {
                    return "parentId";
                }
                return super.getColName(filterName);
            }
        },
        COORD{
            public Set<String> getFilterNames() {
        ...
        public String getColName(String filterName) {
            String colName = filterName;
            if (filterName.equals(OozieClient.FILTER_NAME)) {
                colName = "appName";
            } else if (filterName.equals(OozieClient.FILTER_STATUS)) {
                colName = "statusStr";
            } else if (filterName.equals(OozieClient.FILTER_UNIT)) {
                colName = "timeUnitStr";
            }
            return colName;
        }
